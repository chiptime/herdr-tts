...
herdr-tts --reader-auto        # Toggle auto-opening the live reader popup on reads (TTS_READER_AUTO)
...
| `r` | ⚙️ Lectura automática | auto-lectura, alcance, debounce, popup lector |
...
| `p` | Popup del lector | `off` ↔ `on` (con `on`, cada lectura en vivo abre el popup del lector; los replays de `--play-file` nunca lo abren); persiste en `TTS_READER_AUTO` — el mismo knob de `--reader-auto` |
