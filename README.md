...
| `t` | 🎨 Apariencia | tema del terminal (oscuro / claro) |
...
| Tecla | Ajuste | Ciclo |
|---|---|---|
| `t` | Tema del terminal | `dark` ↔ `light`; con `dark` (por defecto) la salida es idéntica byte a byte a la de siempre, y `light` ajusta los colores para fondos claros (atenuado → negro, cian → azul, amarillo → negrita+amarillo); el roster de chats adopta el tema del plugin use el perfil que use cada panel de agente; el propio popup re-renderiza al instante y la elección persiste en `TTS_THEME` |
...
* Los cambios aplican a **nuevos procesos**: `p`, `g`, `i`, `f`, `b`, `d`, `s` y `w` aplican al reiniciar el daemon — ahora con una tecla (`R` en el índice de Ajustes) o `herdr-tts --restart-daemon`. Las excepciones son `v`, `n`, `u`, `r` y `t`: se consultan en cada evento o frame, así que cambian **al vuelo**, sin reiniciar. El idioma de la interfaz (`l`) también cambia al vuelo **dentro del popup** (re-renderiza al instante); el resto de procesos lo aplican al arrancar, con precedencia `HERDR_TTS_LANG` de entorno > `config.env` > `en`. El tema (`t` en Apariencia) va más lejos que `l`: el propio popup re-renderiza al instante con el nuevo tema y el **dashboard en marcha lo adopta en el siguiente fotograma** (re-lee `TTS_THEME` de `config.env` en cada frame); el resto de superficies lo aplican al arrancar, con la misma precedencia para `TTS_THEME`.
